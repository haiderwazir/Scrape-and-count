class Crawler
	...
	def self.crawl(url, recursive = 0)
		...
		page = open_page(url)
		url = page.base_uri.to_s.chomp("/") if page
		...
		if !@@visited_links.include?(url) && recursive <= 2 
			if source
				...
				workers_2 = (0..12).map do
			    Thread.new do
			    	begin
			        while link = work_q2.pop(true)
                print "."
  							self.crawl(link, recursive+1) if @@total_links <= 50
  						end
  					rescue ThreadError
  					end
					end
		...
		if recursive == 0
			@@result.each { |page| puts "#{page[:url]} - #{page[:number_of_inputs]}" }
	...
	def self.open_page(url)
		open(url, {ssl_verify_mode: OpenSSL::SSL::VERIFY_NONE}) rescue nil
	end
